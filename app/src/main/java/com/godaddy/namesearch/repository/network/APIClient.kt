...
object APIClient {
    val postClient: APIInterfacePOST
        get() {
            ...
            val builder = Retrofit.Builder()
                ...
                .client(client)
                .baseUrl(BASE_URL)
            return builder.build().create(APIInterfacePOST::class.java)
        }
    ...
    val getExactList: APIInterfaceExactGET
        get() {
            val interceptor = HttpLoggingInterceptor(object : HttpLoggingInterceptor.Logger {
                override fun log(message: String) {
                    LogUtils.D(APIClient::class.java.simpleName, LogUtils.FilterTags.withTags(LogUtils.TagFilter.API), String.format("okHttp logging interceptor=%s", message))
                }
            })
            interceptor.level = HttpLoggingInterceptor.Level.BASIC  // BASIC or BODY
            val client = OkHttpClient.Builder()
